...
function hasClosureCompilerTypeCastComment(text, path) {
  ...
  function hasTypeCastComment(node) {
    return (node.comments &&
            node.comments.some(comment => comment.leading &&
...
function needsParens(path, options) {
  ...
  // anything after. So an expression like `export default
  ...
  case "TSParenthesizedType": {
     ...
     *                 ^          ^
     */
    if (getUnparenthesizedNode(node).type === "TSFunctionType" &&
    ...
    if (node.typeAnnotation.type === "TSParenthesizedType" &&
        ...
        parent.type !== "TSIntersectionType" && parent.type !== "TSUnionType") {
      return false;
    }
    return true;
  }
  ...
  case "FunctionExpression":
    ...
    case "CallExpression":
      return name ===
             "callee"; // Not strictly necessary, but it's clearer to the reader
                       // if IIFEs are wrapped in parentheses.
    case "TaggedTemplateExpression":
  ...
  case "TSNonNullExpression":
    if ((parent.type === "BindExpression" || parent.type === "NewExpression") &&
        name === "callee" && parent.callee === node) {
      let object = node;
      while (object) {
        switch (object.type) {
        case "CallExpression":
  ...
  case "BindExpression":
    if ((parent.type === "BindExpression" && name === "callee" &&
         parent.callee === node) ||
