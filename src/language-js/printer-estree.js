...
const {printHtmlBinding, isVueEventBindingExpression} =
 ...
 * @property {string} testNodePropertyName - The property at which the test node
 * can be found on the main node, eg "test".
 * @property {boolean} breakNested - Whether to break all nested ternaries when
 * one breaks.
...
function printTernaryOperator(path, options, print, operatorOptions) {
  ...
  if (operatorOptions.shouldCheckJsx &&
      (isJSXNode(testNode) || isJSXNode(consequentNode) ||
    ...
    const wrap = doc => concat([
      ifBreak("(", ""), indent(concat([ softline, doc ])), softline,
      ifBreak(")", "")
    ]);
    ...
    // * null
    const isNull = node => node.type === "NullLiteral" ||
  ...
  //   : c
  // ).call()
  const breakClosingParen = !jsxMode &&
                            (parent.type === "MemberExpression" ||
                             parent.type === "OptionalMemberExpression") &&
                            !parent.computed;

  return maybeGroup(concat([].concat(
      (testDoc =>
           /**
            *     a
            *       ? b
...
function printPathNoParens(path, options, print, args) {
  ...
  case "NGPipeExpression": {
    ...
    const chain = group(concat([
                          ...
                          parts.length > 0 ? parts[0] : "", indent(rest)
                        ]),
                        {id : groupId});
  ...
  case "TSTypeAssertion": {
    ...
    const exprContents = concat([
      ifBreak("("),
      indent(concat([ softline, path.call(print, "expression") ])), softline,
      ifBreak(")")
    ]);
  ...
  case "Identifier": {
    return concat([
      n.name, printOptionalToken(path),
  ...
  case "ArrowFunctionExpression": {
    ...
    const dangling = comments.printDanglingComments(
        ...
        /* sameIndent */ true, comment => {
          const nextCharacter = getNextNonSpaceNonCommentCharacterIndex(
              options.originalText, comment, options);
    ...
    return group(concat([
      concat(parts), group(concat([
        indent(concat([
          line, shouldAddParens ? ifBreak("", "(") : "", body,
  ...
  case "ImportDeclaration": {
    ...
    if (n.specifiers && n.specifiers.length > 0) {
      ...
      if (grouped.length === 1 && standalones.length === 0 && n.specifiers &&
        ...
        parts.push(concat([
          "{", options.bracketSpacing ? " " : "", concat(grouped),
  ...
  case "ReturnStatement":
    ...
    if (n.argument) {
      if (returnArgumentHasLeadingComment(options, n.argument)) {
        ...
        parts.push(group(concat([
          ifBreak(" (", " "),
          indent(concat([ softline, path.call(print, "argument") ])), softline,
          ifBreak(")")
        ])));
      } else {
        parts.push(" ", path.call(print, "argument"));
  ...
  case "CallExpression": {
    ...
    if (
      ...
      return concat([
        isNew ? "new " : "", path.call(print, "callee"), optional,
  ...
  case "ObjectTypeInternalSlot":
    return concat([
      n.static ? "static " : "", "[[", path.call(print, "id"), "]]",
      printOptionalToken(path), n.method ? "" : ": ", path.call(print, "value")
    ]);
  ...
  case "TSTypeLiteral": {
    ...
    if (props.length === 0) {
      if (!hasDanglingComments(n)) {
        return concat([
          leftBrace, rightBrace, printTypeAnnotation(path, options, print)
        ]);
      }

      content = group(concat([
        leftBrace, comments.printDanglingComments(path, options), softline,
        rightBrace, printOptionalToken(path),
        printTypeAnnotation(path, options, print)
      ]));
    } else {
      content = concat([
        ...
        printOptionalToken(path), printTypeAnnotation(path, options, print)
      ]);
    }
    ...
    if ((n.type === "ObjectPattern" && parent && shouldHugArguments(parent) &&
         !n.decorators && parent.params[0] === n) ||
        (shouldHugType(n) && parentParentParent &&
  ...
  case "Property":
    ...
    if (n.shorthand) {
      ...
      parts.push(printAssignment(n.key, printedLeft, ":", n.value,
                                 path.call(print, "value"), options));
    }
  ...
  case "Decorator":
    return concat(
        [ "@", path.call(print, "expression"), path.call(print, "callee") ]);
  ...
  case "ArrayPattern":
    if (n.elements.length === 0) {
      if (!hasDanglingComments(n)) {
        ...
        parts.push(group(concat([
          "[", comments.printDanglingComments(path, options), softline, "]"
        ])));
      }
    } else {
      ...
      parts.push(group(concat([
        "[",
        indent(concat(
            [ softline, printArrayItems(path, options, "elements", print) ])),
  ...
  case "BigIntLiteral":
    return concat([
      printNumber(n.extra ? n.extra.rawValue : // TypeScript
                      n.value),
      "n"
    ]);
  ...
  case "VariableDeclaration": {
    ...
    parts = [
      isNodeStartingWithDeclare(n, options) ? "declare " : "", n.kind,
  ...
  case "IfStatement": {
    ...
    const opening = group(concat([
      "if (", group(concat([
        indent(concat([ softline, path.call(print, "test") ])), softline
      ])),
      ")", con
    ]));
  ...
  case "ForStatement": {
    ...
    return concat([
      printedComments, group(concat([
        "for (", group(concat([
          indent(concat([
            softline, path.call(print, "init"), ";", line,
            path.call(print, "test"), ";", line, path.call(print, "update")
          ])),
          softline
        ])),
        ")", body
  ...
  case "ForInStatement":
    ...
    return group(concat([
      n.each ? "for each (" : "for (", path.call(print, "left"), " in ",
  ...
  case "ForAwaitStatement": {
    ...
    return group(concat([
      "for", isAwait ? " await" : "", " (", path.call(print, "left"), " of ",
      path.call(print, "right"), ")",
      adjustClause(n.body, path.call(print, "body"))
    ]));
  }
  ...
  case "LabeledStatement":
    ...
    return concat(
        [ path.call(print, "label"), ": ", path.call(print, "body") ]);
  case "TryStatement":
    return concat([
      "try ", path.call(print, "block"),
  ...
  case "SwitchStatement":
    return concat([
      ...
      n.cases.length > 0 ? indent(concat([
        hardline, join(hardline, path.map(
                                     casePath => {
    ...
    return concat([
      ...
      path.call(
          p => {
            ...
            return concat([
              indent(concat([
                softline, comments.printComments(p, () => printed, options)
              ])),
              softline
            ]);
          },
  ...
  case "JSXExpressionContainer": {
    ...
    const preventInline = parent.type === "JSXAttribute" &&
    ...
    const shouldInline =
        ...
        (n.expression.type === "ArrayExpression" ||
         ...
         (isJSXNode(parent) && (n.expression.type === "ConditionalExpression" ||
  ...
  case "JSXOpeningElement": {
    ...
    if (n.selfClosing && !n.attributes.length && !nameHasComments) {
      return concat([
        "<", path.call(print, "name"), path.call(print, "typeParameters"), " />"
      ]);
    }
    ...
    if (n.attributes && n.attributes.length === 1 && n.attributes[0].value &&
      ...
      return group(concat([
        "<", path.call(print, "name"), path.call(print, "typeParameters"), " ",
        concat(path.map(print, "attributes")), n.selfClosing ? " />" : ">"
      ]));
    }
    ...
    return group(concat([
                   ...
                   path.call(print, "typeParameters"), concat([
                     indent(concat(path.map(
                         attr => concat([ line, print(attr) ]), "attributes"))),
  ...
  case "JSXClosingFragment": {
    ...
    return concat([
      isOpeningFragment ? "<" : "</", indent(concat([
  ...
  case "TemplateLiteral": {
    ...
    if (isSimple) {
      expressions = expressions.map(
          doc => printDocToString(doc, Object.assign({}, options, {
                   printWidth : Infinity
                 })).formatted);
    }
  ...
  case "TaggedTemplateExpression":
    return concat([
      path.call(print, "tag"), path.call(print, "typeParameters"),
  ...
  case "TSDeclareFunction":
    ...
    return concat([
      n.declare ? "declare " : "",
      printFunctionDeclaration(path, print, options), semi
    ]);
  case "DeclareFunction":
    return printFlowDeclaration(path, [
      "function ", path.call(print, "id"), n.predicate ? " " : "",
      path.call(print, "predicate"), semi
    ]);
  case "DeclareModule":
  ...
  case "DeclareModuleExports":
    return printFlowDeclaration(
        path,
        [ "module.exports", ": ", path.call(print, "typeAnnotation"), semi ]);
  ...
  case "FunctionTypeParam":
    return concat([
      path.call(print, "name"), printOptionalToken(path), n.name ? ": " : "",
      path.call(print, "typeAnnotation")
    ]);
  ...
  case "TSClassImplements":
    return concat(
        [ path.call(print, "expression"), path.call(print, "typeParameters") ]);
  ...
  case "UnionTypeAnnotation": {
    ...
    if (n.type === "TSUnionType") {
      ...
      hasParens = (parent.type === "TSParenthesizedType" &&
  ...
  case "ObjectTypeIndexer": {
    const variance = getFlowVariance(n);
    return concat([
      variance || "", "[", path.call(print, "id"), n.id ? ": " : "",
  ...
  case "ObjectTypeProperty": {
    ...
    return concat([
      modifier, isGetterOrSetter(n) ? n.kind + " " : "", variance || "",
      printPropertyKey(path, options, print), printOptionalToken(path),
      isFunctionNotation(n, options) ? "" : ": ", path.call(print, "value")
    ]);
  }
  ...
  case "TypeCastExpression": {
    ...
    // Flow supports a comment syntax for specifying type annotations:
    // https://flow.org/en/docs/types/comments/. Unfortunately, its parser
    // doesn't differentiate between comment annotations and regular annotations
    // when producing an AST. So to preserve parentheses around type casts that
    // use the comment syntax, we need to hackily read the source itself to see
    // if the code contains a type annotation comment.
    //
    // Note that we're able to use the normal whitespace regex here because the
    // Flow parser has already deemed this AST node to be a type cast. Only the
    // Babel parser needs the non-line-break whitespace regex, which is why
    // hasFlowShorthandAnnotationComment() is implemented differently.
    const commentSyntax =
        value && value.typeAnnotation && value.typeAnnotation.range &&
        options.originalText.substring(value.typeAnnotation.range[0])
            .match(/^\/\*\s*:/);
    return concat([
      "(", path.call(print, "expression"), commentSyntax ? " /*" : "", ": ",
      path.call(print, "typeAnnotation"), commentSyntax ? " */" : "", ")"
    ]);
  }
  ...
  case "TypeParameterInstantiation": {
    ...
    const commentStart = value.range
                             ? options.originalText.substring(0, value.range[0])
                                   .lastIndexOf("/*")
                             : -1;
  ...
  case "TypeParameter": {
    ...
    const grandParent = path.getNode(2);
    if (parent.params && parent.params.length === 1 && options.filepath &&
        /\.tsx$/i.test(options.filepath) && !n.constraint &&
  ...
  case "TSAsExpression":
    return concat([
      path.call(print, "expression"), " as ", path.call(print, "typeAnnotation")
    ]);
  case "TSArrayType":
    return concat([ path.call(print, "elementType"), "[]" ]);
  ...
  case "TSIndexSignature": {
    ...
    return concat([
      ...
      n.static ? "static " : "", n.readonly ? "readonly " : "", "[",
  ...
  case "TSTypePredicate":
    return concat([
      path.call(print, "parameterName"), " is ",
      path.call(print, "typeAnnotation")
    ]);
  ...
  case "TSImportType":
    return concat([
      !n.isTypeOf ? "" : "typeof ", "import(", path.call(print, "parameter"),
  ...
  case "TSIndexedAccessType":
    return concat([
      path.call(print, "objectType"), "[", path.call(print, "indexType"), "]"
    ]);
    ...
    return group(
        concat([
          "{", indent(concat([
            ...
            ": ", path.call(print, "typeAnnotation"), ifBreak(semi, "")
          ])),
          comments.printDanglingComments(path, options, /* sameIndent */ true),
          options.bracketSpacing ? line : softline, "}"
        ]),
  ...
  case "TSEnumDeclaration":
    ...
    if (n.members.length === 0) {
      ...
      parts.push(group(concat([
        "{", indent(concat([
          hardline, printArrayItems(path, options, "members", print),
  ...
  case "TSConditionalType":
    return printTernaryOperator(path, options, print, {
      beforeParts : () => [path.call(print, "checkType"), " ", "extends", " ",
  ...
  case "NGMicrosyntaxKeyedExpression": {
    ...
    return concat([
      path.call(print, "key"), shouldNotPrintColon ? " " : ": ",
      path.call(print, "expression")
    ]);
  }
  case "NGMicrosyntaxLet":
    return concat([
      "let ", path.call(print, "key"),
      n.value === null ? "" : concat([ " = ", path.call(print, "value") ])
    ]);
  case "NGMicrosyntaxAs":
    return concat(
        [ path.call(print, "key"), " as ", path.call(print, "alias") ]);
  default:
    /* istanbul ignore next */
...
function printPropertyKey(path, options, print) {
  ...
  if (options.quoteProps === "consistent" && !needsQuoteProps.has(parent)) {
    const objectHasStringProp =
        (parent.properties || parent.body || parent.members)
            .some(prop => !prop.computed && prop.key &&
  ...
  if (key.type === "Identifier" && !node.computed &&
    ...
    return path.call(
        keyPath => comments.printComments(keyPath, () => prop, options), "key");
  }

  if (!node.computed && isStringPropSafeToCoerceToIdentifier(node, options) &&
                   ...
                   row => join(
                       ...
                       row.cells.map(
                           (cell, index) =>
                               row.hasLineBreak
                                   ? cell
                                   : cell + " ".repeat(maxColumnWidths[index] -
                                                       getStringWidth(cell))))))
        ])),
        hardline, "`");
    return concat(parts);
...
function printArgumentsList(path, options, print) {
  ...
  function hasEmptyLineInObjectArgInArrowFunction(arg) {
    return (
        arg && arg.type === "ArrowFunctionExpression" && arg.params &&
        arg.params.some(param => param.type && param.type === "ObjectPattern" &&
                                 param.properties &&
  ...
  const contents = concat([
    ...
    ifBreak(maybeTrailingComma), softline, ")"
  ]);
...
function printFunctionParams(path, print, options, expandArg, printTypeParams) {
  ...
  // the params of the first/last argument, we dont want the arguments to break
  ...
  if (shouldExpandParameters) {
    return group(concat([
      removeLines(typeParams), "(", concat(printed.map(removeLines)), ")"
    ]));
  }
  ...
  return concat([
    typeParams, "(", indent(concat([ softline, concat(printed) ])),
...
function printObjectMethod(path, options, print) {
  ...
  if (objMethod.method || objMethod.kind === "get" ||
      objMethod.kind === "set") {
    return printMethod(path, options, print);
  }
    ...
    if (decl.specifiers && decl.specifiers.length > 0) {
      ...
      if (specifiers.length !== 0) {
        if (canBreak) {
          ...
          printed = concat([
            "{", options.bracketSpacing ? " " : "", concat(specifiers),
            options.bracketSpacing ? " " : "", "}"
          ]);
        }
      }
...
function printTypeParameters(path, options, print, paramsKey) {
  ...
  const shouldInline =
      ...
      (n[paramsKey].length === 1 &&
       (shouldHugType(n[paramsKey][0]) ||
        ...
        (greatGreatGrandParent &&
         greatGreatGrandParent.type === "VariableDeclarator" && grandparent &&
  ...
  return group(concat([
    "<",
    indent(concat(
        [ softline, join(concat([ ",", line ]), path.map(print, paramsKey)) ])),
    ifBreak(options.parser !== "typescript" && shouldPrintComma(options, "all")
                ? ","
                : ""),
    softline, ">"
  ]));
...
function printClass(path, options, print) {
  ...
  if (n.superClass) {
    const printed = concat([
      "extends ", path.call(print, "superClass"),
    ...
    if ((!n.implements || n.implements.length === 0) &&
      ...
      partsGroup.push(group(concat([
        line, path.call(superClass => comments.printComments(
                            superClass, () => printed, options),
                        "superClass")
      ])));
    }
  ...
  if (n["implements"] && n["implements"].length > 0) {
    partsGroup.push(
        line, "implements", group(indent(concat([
          line, join(concat([ ",", line ]), path.map(print, "implements"))
        ]))));
  }
...
function printMemberChain(path, options, print) {
  ...
  function shouldInsertEmptyLineAfter(node) {
    ...
    const nextCharIndex =
        getNextNonSpaceNonCommentCharacterIndex(originalText, node, options);
  ...
  const flatGroups =
      groups.slice(0, cutoff).reduce((res, group) => res.concat(group), []);
  ...
  if (hasComment || callExpressions.length >= 3 ||
...
function printJSXElement(path, options, print) {
  ...
  if (n.type === "JSXElement" && isEmptyJSXElement(n)) {
    return concat([
      path.call(print, "openingElement"), path.call(print, "closingElement")
    ]);
  }
  ...
  if (n.children.length === 1 &&
    ...
    return concat(
        [ openingLines, concat(path.map(print, "children")), closingLines ]);
  }
  ...
  const multiLineElem = group(concat([
    openingLines, indent(concat([ hardline, content ])), hardline, closingLines
  ]));
...
function maybeWrapJSXElementInParens(path, elem) {
  ...
  const shouldBreak = matchAncestorTypes(path, [
    "ArrowFunctionExpression", "CallExpression", "JSXExpressionContainer"
  ]);

  return group(
      concat([
        ifBreak("("), indent(concat([ softline, elem ])), softline, ifBreak(")")
      ]),
      {shouldBreak});
}
  ...
  if (isBinaryish(node)) {
    ...
    const right =
        shouldInline
            ...
            : concat([
                lineBeforeOperator ? softline : "", operator,
                lineBeforeOperator ? " " : line, path.call(print, "right"),
                rightSuffix
              ]);
...
function shouldHugType(node) {
  ...
  if (node.type === "UnionTypeAnnotation" || node.type === "TSUnionType") {
    ...
    const objectCount =
        node.types
            .filter(n => n.type === "ObjectTypeAnnotation" ||
...
function shouldHugArguments(fun) {
  return (
      fun && fun.params && fun.params.length === 1 && !fun.params[0].comments &&
      (fun.params[0].type === "ObjectPattern" ||
       fun.params[0].type === "ArrayPattern" ||
       (fun.params[0].type === "Identifier" && fun.params[0].typeAnnotation &&
